...
(function (Manager) {
    ...
    Manager.machine = (0, xstate_1.createMachine)(
    //----------------------------------------------------------------------------------------------------------------//
    //  Config                                                                                                        //
    //----------------------------------------------------------------------------------------------------------------//
    {
        ...
        on: {
            ...
            REGISTER_DESTINATION: { actions: ((context, { type, $, ...event }) => {
                    ...
                    if (destinations.has(destinationID)) {
                        throw new index_1.Error.Duplicate_DestinationID({ destinationID });
                    }
                    const service = (0, xstate_1.interpret)(Destination_1.Destination.machine.withConfig({}, event));
                    service.start();
            ...
            REGISTER_ENTRANCE: { actions: ((context, { type, $, ...event }) => {
                    ...
                    if ([...entranceGroups.values()].some(group => group.has(entranceID))) {
                        throw new index_1.Error.Duplicate_EntranceID({ entranceID });
                    }
                    const service = (0, xstate_1.interpret)(Entrance_1.Entrance.machine.withConfig({}, event));
                    service.start();
            ...
            REGISTER_TRANSPORT: { actions: ((context, { type, $, ...event }) => {
                    ...
                    const { destinationID, entranceID } = event;
                    //###  Register  ###//
                    const service = (0, xstate_1.interpret)(Transport_1.Transport.machine.withConfig({}, event));
                    service.start();
    ...
    //----------------------------------------------------------------------------------------------------------------//
    {
        actions: {
            ...
            throw_DestinationOccupied_Error(context, event) {
                throw new index_1.Error.Destination_Occupied({ destinationID: event.destinationID });
            },
        },
        guards: {
